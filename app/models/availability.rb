...
class Availability < ActiveRecord::Base
	...
	def find_overlap(baseline) #Determine if an availability overlaps with a baseline availability and if so, return the start and end datetime's for the overlap window.
		shared_games = baseline.find_shared_games(self)
		unless self.start_at > baseline.end_at or self.end_at < baseline.start_at or self.id == baseline.id or shared_games.empty?
			...
			if (overlap_end - overlap_start) >= 1.hour
				...
				overlap_availability.games = shared_games
	...
	def find_shared_games(other_availability)
		self.games.select {|game| other_availability.games.include?(game)}
	end
end
